...
PRIVATE phys_clicks clickscdmj; /* store requested amount of memory*/
 ...
 */
  ...
  phys_clicks old_base;
	phys_clicks biggest;/*To store the biggest hole found while traversing*/
  clickscdmj = clicks;
  if (algorithm == 0){
/*	printf("clicks first fit: %d\n",clicks);*/
    do {
	hp = hole_head;
...
/*	printf("clicks %d\n ",clicks);*/
	...
	while (hp != NIL_HOLE && hp->h_base < swap_base){/*traverse the list in order to find the biggest hole*/
		if (hp->h_len>biggest){
			biggest = hp->h_len;
